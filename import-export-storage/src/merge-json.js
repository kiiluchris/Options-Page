 ...
 * @returns 
 */
export function mergeJSONObjects(old = {}, data = {}, {filterKeys={}} = {}){
  ...
  for(const key of keys){
    ...
    result[key] = result.hasOwnProperty(key) ?
      mergeOrReplaceVariable(result[key], currentNew, {key, filterKeys}) : currentNew;
  }
...
function checkItemInArray({key = '', subKey = '', item = [], arr = []}){
  ...
  for(const subItem of item[key]){
    for(let subIndex = 0; subIndex < arr.length; subIndex++){
      ...
      if(
        Array.isArray(oldItem[key]) &&
        ~(index = oldItem[key].findIndex(_r => _r[subKey] === subItem[subKey]))
      ){
        return {
          isInArray: true,
          oldItem,
 ...
 * @returns 
 */
export function mergeArrays(old = [], arr = [], {key = '', filterKeys = {}} = {}){
  ...
  for(let i = 0; i < arr.length; i++){
    ...
    if(typeof item === 'object' && typeof result[i] === 'object'){
      ...
      if(!Array.isArray(item) && subKey !== undefined){
        if((
            item[key] !== undefined &&
            item[key][subKey] !== undefined &&
            ~(index = result.findIndex(r => r[key][subKey] === item[key][subKey]))
          ) || (
            item[subKey] !== undefined &&
            ~(index = result.findIndex(r => r[subKey] === item[subKey]))
        )){
          ...
          if(isInArray){
            ...
            delete item[key];
            delete oldItem[key];
            const obj = mergeOrReplaceVariable(
              oldItem, item, {filterKeys}
            );
            obj[key] = mergeOrReplaceVariable(
              oldItemArr, itemArr, {
              key: subKey, filterKeys: {[subKey]:subKey, ...filterKeys}
            });
            result[subIndex] = obj;
            hasChanged = true;
          }
...
export function mergeOrReplaceVariable(oldVal, newVal, {key = '', filterKeys = {}} = {}) {
...
mergeOrReplaceVariable.all = (arr = [], options = {}) => {
  return arr.reduce((prev, next) => (
    mergeOrReplaceVariable(prev, next, options)
  ), {});
};